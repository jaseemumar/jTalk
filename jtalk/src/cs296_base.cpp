...
long long nDestroyed=0;

namespace cs296{
	extern bool createfuel;
	extern bool destroyfuel;
	extern bool braked;
	extern queue<b2Body*> toBeDestroyed;
	extern long long ntoBeDestroyed;
	set<b2Body*> fuelSet;
	set<b2Body*> inCylFuelSet;
}
...
void base_sim_t::step(settings_t* settings)
{
  ...
  while(!toBeDestroyed.empty()){
	  m_world->DestroyBody(toBeDestroyed.front());
	  toBeDestroyed.pop();
  }
 // if(destroyfuel){m_world->DestroyBody(toBeDestroyed);destroyfuel=false;}
  ...
  if(createfuel){
	  for (int i=0;i<2;i++){
		  b2Body *fuel;
			...
			myBodyDef.position.Set(-20+1,20+12);
			b2FixtureDef fd;
			fd.density=5.0f;
			...
			fuel=m_world->CreateBody(&myBodyDef);
			fuel->CreateFixture(&fd);
			fuel->SetLinearVelocity(b2Vec2(5,-5));
			createfuel=false;
			fuelSet.insert(fuel);
		}
  }
