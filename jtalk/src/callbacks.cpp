...
namespace cs296
{
  ...
  void callbacks_t::keyboard_cb(unsigned char key, int x, int y)
  {
    ...
    case 'b':
		...
		brakehandle->ApplyForce(b2Vec2(5000,0), brakehandle->GetWorldCenter(), true);
		braked=true;
	...
	case 's':
		crank->ApplyTorque(-1000, true);
		braked=false;
    	break;

      //! The default case. Why is this needed?
    default:
		brakehandle->ApplyForce(b2Vec2(-500,0), brakehandle->GetWorldCenter(), true);
      if (test)
	{
	  test->keyboard(key);
	}
